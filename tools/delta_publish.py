...
- verify a publication proof offline, without token/blockchain/SaaS dependencies
...
- local_timestamp_v1 records a local timestamp claim. Strong external anchoring
  such as OpenTimestamps/GitHub release/IPFS can be attached as evidence later.
"""
...
def create_publication_body(
    *,
    ...
    publication: dict[str, Any] = {
        ...
        "publisher": publisher,
        "status": "created",
        "verification_mode": "offline_shape_and_hash_binding_v2_0_0",
    }
    if external_uri:
    ...
    body = {
        ...
        "security_boundary": {
            ...
            "note": "Proof of Publication binds a publication claim to a DELTA record hash. External timestamp strength depends on the chosen method/evidence.",
        },
    }
    return body
...
def create_proof(body: dict[str, Any]) -> dict[str, Any]:
    ...
    proof = {
        ...
        "proof_integrity": {
            ...
            "signed": False,
            "signature": None,
            "note": "v2.0.0 MVP uses hash integrity. Optional publication signing may be added later.",
        },
    }
    return proof


def verify_publication_proof(*, proof: dict[str, Any], record: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if publication.get("external_evidence_hash"):
        checks["external_evidence_hash_shape_ok"] = isinstance(publication.get("external_evidence_hash"), str) and publication.get("external_evidence_hash", "").startswith(SHA256_PREFIX)
...
def command_create_proof(args: argparse.Namespace) -> int:
    ...
    body = create_publication_body(
        ...
        external_hash=args.external_hash,
        note=args.note,
    )
...
def command_verify_proof(args: argparse.Namespace) -> int:
    ...
    result = verify_publication_proof(proof=proof, record=record)
